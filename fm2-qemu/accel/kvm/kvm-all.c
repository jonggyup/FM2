...
/* Jonggyu: Unlike upstream kvm_slot_get_dirty_log(), FM2 selects the
 * huge-page ioctl during background FMSync and the split/base-page ioctl at
 * switchover. The returned bitmap is consumed by QEMU's RAMBlock bitmap. */
...
/* Jonggyu: FM2 mirrors QEMU's normal KVM dirty-log listener, but preserves
 * the huge-page granularity expected by the kernel FM2 extension. */
                                             ...
                                             bool switchover)
{
    ...
    while (size) {
        ...
        /* Jonggyu: the kernel rounds the memslot to 2 MiB boundaries; mirror
         * that rounding below when translating the returned bitmap into the
         * RAMBlock bitmap. This is why the FM2 path is not a direct copy of
         * QEMU's ordinary 4 KiB dirty-log synchronization. */
...
/* Jonggyu: this listener is the QEMU-side hook added for FMSync; normal
 * migration continues to use kvm_log_sync(). */
