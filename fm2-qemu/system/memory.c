...
/* Jonggyu: FM2's counterpart to memory_global_dirty_log_sync(). It invokes
 * the FM2 listener for each migratable range and passes switchover so the
 * KVM backend can change dirty-log granularity for the final flush. */
void fmsync_memory_dirty_log_huge(bool last_stage, bool switchover) {
     ...
     */
    QTAILQ_FOREACH(listener, &memory_listeners, link) {
        /* Jonggyu: select only the listener registered by the FM2 KVM path;
         * ordinary migration listeners remain on the upstream callback. */
        as = listener->address_space;
        view = address_space_get_flatview(as);
        FOR_EACH_FLAT_RANGE(fr, view) {
