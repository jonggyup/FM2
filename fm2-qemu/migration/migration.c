...
static inline void cxl_invalidate_range_simple(void *addr, size_t len)
{
    uintptr_t p   = ((uintptr_t)addr) & ~(CL_SIZE - 1);
    uintptr_t end = (uintptr_t)addr + len;
                                        ...
                                        int *current_active_state)
{
    ...
    cxl_flush_range(s->shm_obj.shm_ptr, SHM_MIGRATION_QUEUE_SIZE);
...
{
    /* Jonggyu: FMFlush is entered here after the shared-memory iterate loop
     * reaches the switchover condition; controller notifications delimit the
     * source stop, publication of the CXL image, and destination handoff. */
    int ret = 0;
    int current_active_state = s->state;
...
{
    /* Jonggyu: This loop is the paper's continuous FMSync controller. Each
     * round invokes ram_save_iterate_shm(), then sleeps for the configured
     * interval until either the switchover request or the next round. */
...
out: // clean
    cxl_flush_range(s->shm_obj.shm_ptr, SHM_MIGRATION_QUEUE_SIZE);
    //Jonggyu addition just in case to see if unmap solve the system ram problem.
    if (munmap(s->shm_obj.shm_ptr, s->shm_obj.shm_size) == -1) {
...
extern void fm2_promoter_start(void *devdax_hva_base, uint64_t bytes);
extern void *devdax_base;
extern uint64_t dev_dax_bytes;
...
process_incoming_migration_shm_co(void *opaque)
{
    ...
    // shit copilot.
    assert(mis->from_src_file);
//    qemu_file_clflush_buf(mis->from_src_file);
    printf("qemufile flush \n");
...
void qmp_migrate_incoming_shm(void *shm_ptr, uint64_t shm_size, Error **errp) 
{
    ...
    cxl_invalidate_range_simple(shm_ptr, buf_size);
