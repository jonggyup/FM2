				     ...
				     bool wp_async)
{
	...
	/* Jonggyu: Delta from upstream Linux v6.8.0. FMLift needs UFFD write
	 * protection on DevDAX so QEMU can copy a 2 MiB CXL region and atomically
	 * remap it to DRAM without losing guest writes during the copy. */
...
/* Jonggyu: Delta from upstream Linux v6.8.0. The marker restriction is kept
 * disabled for DevDAX because FMLift relies on the write-protect fault path,
 * not on userfaultfd's anonymous-marker optimization. */
/*static inline bool userfaultfd_wp_use_markers(struct vm_area_struct *vma)
{
    if (!userfaultfd_wp(vma))
        return false;
