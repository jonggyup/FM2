...
/* Jonggyu: Delta from upstream Linux v6.8.0: this separate ioctl path keeps
 * FM2's bitmap and snapshot cadence independent from KVM_GET_DIRTY_LOG. FM2 polls
 * 2 MiB regions while maintaining the shadow copy, then asks for 4 KiB
 * granularity after the final huge-page split before handing off the VM. */
static int kvm_fmsync_get_dirty_log(struct kvm *kvm, struct kvm_dirty_log *log, bool is_huge) {
	...
	as_id = log->slot >> 16;
	id = (u16)log->slot;
	if (as_id >= KVM_MAX_NR_ADDRESS_SPACES || id >= KVM_USER_MEM_SLOTS)
		return -EINVAL;
	...
	/* Jonggyu: round the slot to huge-page boundaries so the bitmap remains
	 * well-defined even if QEMU registered an unaligned memory slot. */
	...
	if (!memslot->fmsync_dirty_bitmap) {
		...
		/*for (unsigned long i = 0; i < n / 8; ++i) {
			memslot->fmsync_dirty_bitmap[i] = 0;
		} */
        memset(memslot->fmsync_dirty_bitmap, 0, n);
	}

	/* Match KVM's dirty-log snapshot ordering: flush translations, take the
	 * architecture-specific snapshot under the MMU lock, then flush again. */
	kvm_flush_remote_tlbs_memslot(kvm, memslot);
	// pull the dirty info
	KVM_MMU_LOCK(kvm);
			   ...
			   unsigned int ioctl, unsigned long arg)
{
	...
	case KVM_FMSYNC_GET_DIRTY_LOG_HUGE: {
		...
		r = -EFAULT;
		/* Jonggyu: copy the ioctl payload before selecting FM2's granularity. */
		if (copy_from_user(&log, argp, sizeof(log)))
			goto out;
	...
	case KVM_FMSYNC_GET_DIRTY_LOG_BASE_WITH_SPLIT: {
		...
		r = -EFAULT;
		/* Jonggyu: the switchover ioctl uses the same ABI with base-page output. */
		if (copy_from_user(&log, argp, sizeof(log)))
			goto out;
