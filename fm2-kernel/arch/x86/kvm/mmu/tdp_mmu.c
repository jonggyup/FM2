 ...
 * write-protect them, and return whether any pages were dirtied.
 *
 * When called repeatedly until it returns false, all pages are guaranteed
 * clean because:
 ...
 */
/* Jonggyu: Delta from upstream Linux v6.8.0. FMSync walks the same TDP leaf
 * SPTEs as KVM's ordinary dirty logging, but exports either one bit per 2 MiB
 * region or one bit per 4 KiB page and clears dirty/write state for the next
 * epoch. This is the kernel half of the paper's huge-page tracking and
 * switchover split; QEMU consumes the resulting bitmap in ram.c. */
bool kvm_tdp_mmu_fmsync_dirty_log(struct kvm *kvm,
				  const struct kvm_memory_slot *slot,
				  bool is_huge)
{
...
//        kvm_mmu_try_split_huge_pages(kvm, slot, start_gfn, end_gfn, PG_LEVEL_4K);
    ...
    for_each_valid_tdp_mmu_root_yield_safe(kvm, root, slot->as_id) {
        ...
        tdp_root_for_each_leaf_pte(iter, root, walk_lo, walk_hi) {
            ...
            if (iter.level != PG_LEVEL_4K &&
                (!is_huge || iter.level == PG_LEVEL_1G)) {
            ...
            if (is_huge) {
                ...
                idx = leaf_start - start_gfn;
                bitmap_set(slot->fmsync_dirty_bitmap,
                           idx, (unsigned long)(leaf_end - leaf_start));
            }

            new_spte = spte_ad_enabled(iter.old_spte) ?
