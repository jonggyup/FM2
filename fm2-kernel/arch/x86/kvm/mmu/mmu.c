...
/* Jonggyu: Delta from upstream Linux v6.8.0. Keep the architecture hook
 * narrow: the generic KVM ioctl owns locking/copy-to-userspace, while the
 * x86 TDP implementation performs the page-table walk and bitmap update. */
void kvm_arch_mmu_fmsync_dirty_log(struct kvm *kvm, struct kvm_memory_slot *memslot, bool is_huge) {
	kvm_tdp_mmu_fmsync_dirty_log(kvm, memslot, is_huge);
	return;
}
